...
@implementation NSString (Rational)
...
- (NSString *)fail:(NSBundle *)bundle localized:(NSString *)table {
    //: if (!bundle) {
    if (!bundle) {
        //: return self;
        return self;
...
- (NSString *)monthAddress:(NSString *)table {
    //: NSBundle * languageBundle = [AppleProjectKit sharedKit].languageBundle;
    NSBundle * languageBundle = [Rational coordinator].languageBundle;
    //: return [self nim_localizedByBundle:languageBundle table:table];
    return [self fail:languageBundle localized:table];
}
