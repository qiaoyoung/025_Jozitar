...
static NSString *componentCapabilityKey = @"UITableViewCell";

//: @interface ZZZCommonTableDelegate()
@interface CursiveDelegate()
...
@implementation CursiveDelegate
...
- (instancetype) initWithAfterCalendar:(NSArray *(^)(void))data{
    ...
    if (self) {
        ...
        _NTESDataReceiver = data;
	[self setSignature:_defaultSeparatorLeftEdge];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    MatterFlatWe *tableSection = self.suggest[section];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MatterFlatWe *tableSection = self.suggest[indexPath.section];
    //: NIMCommonTableRow *tableRow = tableSection.rows[indexPath.row];
    SegmentSlide *tableRow = tableSection.rows[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *identity = tableRow.cellClassName.length ? tableRow.cellClassName : componentCapabilityKey;
    ...
    if (![cell respondsToSelector:@selector(angle:medication:)]) {
        //: UITableViewCell *defaultCell = (UITableViewCell *)cell;
        UITableViewCell *defaultCell = (UITableViewCell *)cell;
        //: [self refreshData:tableRow cell:defaultCell];
        [self opinion:tableRow assemblage_strong:defaultCell];
    //: }else{
    }else{
        //: [(id<ZZZCommonTableViewCell>)cell refreshData:tableRow tableView:tableView];
        [(id<GradSlideQuantityroduce>)cell angle:tableRow medication:tableView];
    }
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //: }else{
        ...
        MatterFlatWe *section = self.suggest[indexPath.section];
        ...
        //: }else{
            ...
            sepWidth = cell.device_width - [self scheme:self.defaultSeparatorLeftEdge];
        }
    }
...
- (CGFloat)scheme:(CGFloat)signature {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!tableRow.forbidSelect) {
        ...
        UIViewController *vc = tableView.signatureWorldView;
...
- (void)opinion:(SegmentSlide *)rowData assemblage_strong:(UITableViewCell *)cell{
...
- (void)setSignature:(CGFloat)signature {
    //: OC_CUSTOM_PROPERTY_INJECT
    _signature = signature;
}
