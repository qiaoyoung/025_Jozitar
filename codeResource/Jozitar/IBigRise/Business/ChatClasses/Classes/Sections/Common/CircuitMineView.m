...
- (void)growing;
//: - (void)reloadPage;
- (void)inheritancePage;
//: @end
@end

//: @implementation ZZZPageView
@implementation CircuitMineView
...
- (void)facingPages:(NSInteger)currentPage
{
    ...
    NSInteger first = [self page:currentPage - 1 read:0 globalDownTotalerval:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self page:currentPage + 1 read:0 globalDownTotalerval:count - 1];
    ...
    for (NSUInteger index = 0; index < count; index++)
    {
        ...
        if (NSLocationInRange(index, range))
        {
            ...
            if (![obj isKindOfClass:[UIView class]])
            {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(keepRender:list:)])
                {
                    //: UIView *view = [_dataSource pageView:self viewInPage:index];
                    UIView *view = [_dataSource keepRender:self list:index];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (page >= 0 && page < [_pages count])
    {
        ...
        [self facingPages:_extent];
    }

    //: if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(pageViewDidScroll:)])
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(sequences:)])
    {
        //: [_pageViewDelegate pageViewDidScroll:self];
        [_pageViewDelegate sequences:self];
    }
}
...
- (void)address: (NSInteger)page
{
    //: if (_currentPage != page || page == 0)
    if (_extent != page || page == 0)
    {
        //: _currentPage = page;
        _extent = page;
        //: [self reloadData];
...
- (void)layoutSubviews
{
    ...
    if (self.pageViewDelegate && [self.pageViewDelegate respondsToSelector:@selector(absoluteDismiss)])
    {
        //: animation = [self.pageViewDelegate needScrollAnimation];
        animation = [self.pageViewDelegate absoluteDismiss];
    }
    //: [self.scrollView scrollRectToVisible:CGRectMake(_currentPage * size.width, 0, size.width, size.height)
    [self.scrollView scrollRectToVisible:CGRectMake(_extent * size.width, 0, size.width, size.height)
                                //: animated:animation];
                                animated:animation];
...
- (void)growing
{
    ...
    if(_dataSource && [_dataSource respondsToSelector:@selector(poped:)])
    {
        //: numberOfPages = [_dataSource numberOfPages:self];
        numberOfPages = [_dataSource poped:self];
    }
...
- (void)follow:(UIInterfaceOrientation)toInterfaceOrientation
                                         ...
                                         agreementTimeCounterval:(NSTimeInterval)duration
{
    //: CGSize size = self.bounds.size;
    CGSize size = self.bounds.size;
    ...
    _scrollView.contentOffset = CGPointMake(_to * self.bounds.size.width, 0);
...
- (NSInteger)page:(NSInteger)value read:(NSInteger)min globalDownTotalerval:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)holdForth
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(valid:collectPages:clickPages:)])
    {
        //: [_pageViewDelegate pageViewScrollEnd:self
        [_pageViewDelegate valid:self
                                //: currentIndex:_currentPage
                                collectPages:_extent
                                  //: totolPages:[_pages count]];
                                  clickPages:[_pages count]];
    }
}
...
- (void)step:(UIInterfaceOrientation)toInterfaceOrientation
                                //: duration:(NSTimeInterval)duration
                                toALesserExtent:(NSTimeInterval)duration
{
    //: _scrollView.delegate = nil;
    _scrollView.delegate = nil;
...
- (void)inheritancePage
{
    ...
    if (_extent >= [_pages count])
    {
        //: _currentPage = [_pages count] - 1;
        _extent = [_pages count] - 1;
    }
    //: if (_currentPage < 0)
    if (_extent < 0)
    {
