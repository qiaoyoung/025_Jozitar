...
@implementation SprechstimmeView

//: #pragma mark - Private
#pragma mark - Private
//: - (void)drawWithContent:(CGRect)rect context:(CGContextRef)context{
- (void)pass:(CGRect)rect year:(CGContextRef)context{
...
- (CGSize)withTrunk:(NSString *)badgeValue{
...
- (void)automobileDown:(CGRect)rect disappear:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
...
+ (instancetype)original:(NSString *)badgeValue{
    ...
    SprechstimmeView *instance = [[SprechstimmeView alloc] init];
    //: instance.frame = [instance frameWithStr:badgeValue];
    instance.frame = [instance block:badgeValue];
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self block:badgeValue];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self pass:rect year:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self automobileDown:rect disappear:context];
    }
...
- (CGRect)block:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self withTrunk:badgeValue];
