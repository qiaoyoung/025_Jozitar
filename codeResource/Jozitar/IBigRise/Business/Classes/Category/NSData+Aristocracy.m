...
@implementation NSData (Aristocracy)
...
void messageRc4(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)withKey:(NSString *)key {
    ...
    messageRc4(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
