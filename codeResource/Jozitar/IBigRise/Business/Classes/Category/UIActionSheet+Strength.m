...
static char featureScanKey;


//: @implementation UIActionSheet (NTESBlock)
@implementation UIActionSheet (Strength)
...
- (void)exceptCheckion
{
    ...
    objc_setAssociatedObject(self,&featureScanKey,nil,OBJC_ASSOCIATION_COPY);
}
...
- (void)vanguardHandler: (UIView *)view handler: (ActionSheetBlock)block
{
    ...
    objc_setAssociatedObject(self,&featureScanKey,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        UITabBar *tabbar = [self forPresent];
        //: if (tabbar)
        if (tabbar)
        {
        ...
        else
        {
            ...
            [self performSelector:@selector(calendarred:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &featureScanKey) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&featureScanKey,nil,OBJC_ASSOCIATION_COPY);
