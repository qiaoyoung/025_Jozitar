...
#import "UIImage+Aristocracy.h"
//: #import "NTESDevice.h"
#import "WeChunk.h"

//: @implementation UIImage (NTES)
@implementation UIImage (Aristocracy)
//: - (UIImage *)drawImageWithSize: (CGSize)size
- (UIImage *)teamSize: (CGSize)size
{
...
- (UIImage *)vendor: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self wither:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self wither:suggestPixels];
    }
}

//: - (UIImage *)scaleWithMaxPixels: (CGFloat)maxPixels
- (UIImage *)wither: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self visibleEntity:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)visibleEntity:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
...
- (UIImage *)sheetAvatar
{
    ...
    CGFloat pixels = [[WeChunk listing] scaleEven];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self vendor:pixels];
