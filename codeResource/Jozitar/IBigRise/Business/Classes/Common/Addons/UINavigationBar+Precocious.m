...
#import "UINavigationBar+Precocious.h"
...
@implementation UINavigationBar (Precocious)
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationBar class] ,@selector(layoutSubviews), @selector(provideData));
    //: });
    });
}
...
#pragma mark - Private
//: - (void)layoutLabel{
- (void)lawyerClientRelationLabel{
...
- (void)provideData{
    ...
    [label sizeToFit];
    //: [self layoutLabel];
    [self lawyerClientRelationLabel];
}
