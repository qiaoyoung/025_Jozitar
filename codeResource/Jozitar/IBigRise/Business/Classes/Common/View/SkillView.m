...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self smartSuggest:badgeValue];
...
- (CGRect)smartSuggest:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self smartStr:badgeValue];
...
- (void)lockLid:(CGRect)rect to:(CGContextRef)context{
...
- (CGSize)smartStr:(NSString *)badgeValue{
...
- (void)standardise:(CGRect)rect courseOfActionFollow:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
...
+ (instancetype)cut:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
    ...
    SkillView *instance = [[SkillView alloc] init];
    //: instance.frame = [instance frameWithStr:badgeValue];
    instance.frame = [instance smartSuggest:badgeValue];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self lockLid:rect to:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self standardise:rect courseOfActionFollow:context];
    }
