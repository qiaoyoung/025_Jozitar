...
- (void)minIndex:(DownwardChartView *)menuView establishBubbleIndex:(AdvancedView *)menuCell highlight:(NSInteger)index;
...
@interface DownwardChartView : UIView
 ...
 *
 *  @param menuModelsArray     drop-down menu models array. Array storage MagicMarkerBasedModel instances
...
@property (nonatomic, weak) id<VentureWrap> delegate;
 ...
 *               若用框架自带的cell,直接传@"MenuViewCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传@"类名.xib"
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"MenuViewCell"
 *
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down nemu cell's class name. Must be a subclass of AdvancedView
 *              If want to use this framework's drop-down nemu cell, direct pass @"MenuViewCell"
...
@property (nonatomic, assign) CGFloat writingTag;
 ...
 */
//: @property (nonatomic, assign) CGFloat triangleY;
@property (nonatomic, assign) CGFloat sortCover;

///==================================================================
///                    创建下拉菜单方式2
///   适用于自定义下拉菜单样式，单个属性进行赋值
///   >>步骤:
///   1、 [DownwardChartView alloc] init]
///   2、 对需要赋值的属性进行赋值
 ...
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down menu models array(Array storage MagicMarkerBasedModel instances or the MagicMarkerBasedModel's subclass instances )
 */
//: @property (nonatomic, strong) NSArray *menuModelsArray;
@property (nonatomic, strong) NSArray *lookerEnablePlace;
