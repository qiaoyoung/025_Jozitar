...
typedef void(^SGPermissionCameraBlock)(RadioMine *camera, SGPermissionStatus status);

//: @interface SGPermissionCamera : NSObject
@interface RadioMine : NSObject
//: + (void)camera:(SGPermissionCameraBlock)block;
+ (void)regular:(SGPermissionCameraBlock)block;
//: + (void)request:(void (^)(BOOL granted))handler;
+ (void)freshRequest:(void (^)(BOOL granted))handler;
//: @end
@end
