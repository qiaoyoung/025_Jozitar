...
typedef void(^SGPermissionBlock)(VacuumTubePermission *permission, SGPermissionStatus status);

//: @interface SGPermission : NSObject
@interface VacuumTubePermission : NSObject
/// 对象方法获取权限状态
///
/// @param type        权限类型
/// @param block       权限状态回调
...
- (void)lowShadow:(void (^)(BOOL granted))handler;
