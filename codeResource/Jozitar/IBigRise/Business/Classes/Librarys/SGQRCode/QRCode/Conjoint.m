...
@interface Conjoint () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
{
...
@property (nonatomic, strong) AVCaptureDeviceInput *deviceInput;
//: @property (nonatomic, strong) AVCaptureVideoPreviewLayer *videoPreviewLayer;
@property (nonatomic, strong) AVCaptureVideoPreviewLayer *hide;
...
- (AVCaptureDeviceInput *)deviceInput {
    ...
    if (!_deviceInput) {
        ...
        _deviceInput = [AVCaptureDeviceInput deviceInputWithDevice:self.device error:nil];
	[self setHide:_videoPreviewLayer];
    }
    //: return _deviceInput;
    return _deviceInput;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(alongsideSeeResult:added:)]) {
                //: [self.delegate scanCode:self result:resultString];
                [self.delegate alongsideSeeResult:self added:resultString];
            }
        //: });
        });

        //: if ([SGQRCodeLog sharedQRCodeLog].log) {
        if ([OmertaRationalLog writtenLanguage].log) {
        }
    }
}
...
- (void)setPreview:(UIView *)preview {
    ...
    [preview.layer insertSublayer:[self basic:self.videoPreviewLayer] atIndex:0];
}

//: - (void)setSampleBufferDelegate:(id<SGScanCodeSampleBufferDelegate>)sampleBufferDelegate {
- (void)setSampleBufferDelegate:(id<CognizeDelegate>)sampleBufferDelegate {
...
- (void)schedule:(NSString *)name {
    ...
    NSString *voicePath = [[[Join tutorialVertical] hideOdd] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", name]];
    ...
    limit = [FactoryFlat sound:voicePath];
	[self setHide:_videoPreviewLayer];
    //: [soundEffect play];
    [limit reach];
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.sampleBufferDelegate && [self.sampleBufferDelegate respondsToSelector:@selector(ratioPercentage:model:)]) {
            //: [self.sampleBufferDelegate scanCode:self brightness:brightnessValue];
            [self.sampleBufferDelegate ratioPercentage:self model:brightnessValue];
        }
    //: });
    });
}
...
- (void)setHide:(AVCaptureVideoPreviewLayer *)hide {
...
- (AVCaptureVideoPreviewLayer *)basic:(AVCaptureVideoPreviewLayer *)hide {
    //: OC_CUSTOM_PROPERTY_INJECT
    _hide = hide;
    return hide;
}
...
- (void)setDelegate:(id<VentureDelegate>)delegate {
    //: _delegate = delegate;
    _delegate = delegate;
...
- (AVCaptureSession *)session {
	...
	[self setHide:_videoPreviewLayer];
        ...
        _session.sessionPreset = [self resolution];
    }
    //: return _session;
    return _session;
}
...
- (AVCaptureVideoPreviewLayer *)videoPreviewLayer {
    ...
    if (!_videoPreviewLayer) {
        ...
        [self basic:_videoPreviewLayer].frame = self.preview.frame;
    }
    //: return _videoPreviewLayer;
    return [self basic:_videoPreviewLayer];
}
