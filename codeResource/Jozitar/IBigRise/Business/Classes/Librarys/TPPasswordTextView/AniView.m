...
- (void)setPlacement:(UIColor *)placement {
...
- (void)setElementCount:(NSInteger)elementCount {
    ...
    _elementCount = elementCount;
	[self setPlacement:_elementBorderColor];
    //: if (elementCount <= 0) {
    if (elementCount <= 0) {
        //: return;
        return;
    ...
    for (int i = 0; i < self.elementCount; i++)
    {
        ...
        pwdTextField.textAlignment = NSTextAlignmentCenter;
	[self setPlacement:_elementBorderColor];//居中
        //: pwdTextField.secureTextEntry = YES;
        pwdTextField.secureTextEntry = YES;
	[self setInput:_autoHideKeyboard];//设置密码模式
...
- (UIColor *)columniation:(UIColor *)placement {
    //: OC_CUSTOM_PROPERTY_INJECT
    _placement = placement;
    return placement;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetStrokeColorWithColor(context, [self columniation:self.elementBorderColor].CGColor);
    ...
    //: }else {
        ...
        for (NSUInteger i = 0; i < self.dataSource.count; i++) {
            ...
            if (self.dataSource.count - 1 == i) {
                ...
                rightBottomPoint = CGPointMake(right, bottom);
	[self setInput:_autoHideKeyboard];
            //: }else if (0 == i) {
            }else if (0 == i) {
...
- (void)albums:(UITextField *)textField {
    ...
    if (password.length == self.dataSource.count)
    {
        ...
        if ([self bring:self.autoHideKeyboard]) {
            //: [self hideKeyboard];
            [self format];//隐藏键盘
        }
    }
...
- (instancetype)initWithFrame:(CGRect)frame
{
	...
	[self setPlacement:_elementBorderColor];
        ...
        [textField addTarget:self action:@selector(albums:) forControlEvents:UIControlEventEditingChanged];
