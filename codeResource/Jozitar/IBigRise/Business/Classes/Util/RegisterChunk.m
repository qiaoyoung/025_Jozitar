...
@implementation RegisterChunk

//: + (NSString *)bigFileMD5:(NSString *)filePath
+ (NSString *)inundateCondition:(NSString *)filePath
{
...
+ (NSString *)top:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self expected:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self inundateCondition:filepath];
        //: }else {
        }else {
...
+ (void)join:(NSString *)filepath claim:(void(^)(NSString *MD5))completion;
{
    //: dispatch_block_t block = ^(){
    dispatch_block_t block = ^(){
        //: NSString *md5 = [self fileMD5:filepath];
        NSString *md5 = [self top:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (unsigned long long)expected:(NSString *)filepath
{
