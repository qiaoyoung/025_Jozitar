...
@implementation UIImage (Thorough)
...
+ (UIImage *)ating:(NSString *)name {
    ...
    UIImage *image = [[Join tutorialVertical] ating:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self ating:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(ating:));
